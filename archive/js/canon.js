...
//const cannon2=new Cannon("player2Cannon",100,50)
...
playground.div.addEventListener("click", (e) => {
  // Get the bounding box of the playground
  const rect = playground.div.getBoundingClientRect();

  // Calculate the cursor position relative to the canon
  const cursorX = e.clientX - rect.left;
  const cursorY = e.clientY - rect.top;
  //console.log(cursorX/rect.width, cursorY/rect.height)
  // Fire the shot with the calculated cursor position
...
const obstacleInterval = setInterval(() => {
  let obstacleToCreated=numberOfObstacles+numberOfTargets-obstacles.length
  obstacleToCreated=Math.min(obstacleToCreated,2)
  if(obstacleToCreated>=0){
    
    Obstacle.createSeveral(obstacleToCreated)
  }
}, 1000);

// console.log(exclusion_zones)
